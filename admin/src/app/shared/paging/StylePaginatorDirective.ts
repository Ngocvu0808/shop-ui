...
export class StylePaginatorDirective {
	...
	constructor(
		...
		//Sub to rerender buttons when next pages and last pages is used
	...
	private buildPageNumbers() {
		...
		//initialize next pages and last pages buttons
		if (this._buttons.length == 0) {
				...
				for (let i = 0; i < nodeArray.length; i++) {
					if (nodeArray[i].nodeName === 'BUTTON') {
						if (nodeArray[i].disabled) {
							this.ren.setStyle(nodeArray[i], 'background-color', 'rgba(255, 255, 255, 1)');
							this.ren.setStyle(nodeArray[i], 'color', 'rgba(0, 0, 0, 0.26)');
							this.ren.setStyle(nodeArray[i], 'margin', '.1%');
						} else {
							this.ren.setStyle(nodeArray[i], 'background-color', 'rgba(255, 255, 255, 1)');
							this.ren.setStyle(nodeArray[i], 'color', 'rgba(0, 0, 0, 0.54)');
							this.ren.setStyle(nodeArray[i], 'margin', '.1%');
						}
					}
				}
	...
	private createButton(i: any, pageIndex: number): any {
		...
		this.ren.addClass(linkBtn, 'mat-custom-pages');
		switch (i) {
			case pageIndex:
