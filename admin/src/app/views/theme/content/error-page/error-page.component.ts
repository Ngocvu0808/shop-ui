...
export class ErrorPageComponent implements OnInit, OnDestroy {
	...
	constructor(private route: ActivatedRoute, private layoutConfigService: LayoutConfigService) {
		// set temporary values to the layout config on this pages
	...
	ngOnInit() {
		...
		switch (this.type) {
			...
			case 'error-v3':
				...
				if (!this.subtitle) {
					this.subtitle = 'Sorry we can\'t seem to find the pages you\'re looking for.';
				}
				if (!this.desc) {
					this.desc = 'There may be amisspelling in the URL entered,<br>' + 'or the pages you are looking for may no longer exist.';
				}
				if (!this.image) {
